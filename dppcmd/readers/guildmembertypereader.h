#pragma once
#include "typereader.h"
#include <cstdint>
#include <unordered_set>

namespace dpp { class guild_member; class snowflake; }

namespace dppcmd
{
    class guild_member_in : public type_reader<dpp::guild_member>
    {
    public:
        type_reader_result read(dpp::cluster* cluster, const dpp::message_create_t* context,
                                const std::string& input) override;
    protected:
        std::unordered_set<uint64_t> id_set;
        void add_results_by_global_name(const dpp::snowflake guild_id, const std::string& input);
        void add_results_by_id(const dpp::snowflake guild_id, const std::string& input);
        void add_results_by_mention(const dpp::snowflake guild_id, const std::string& input);
        void add_results_by_nickname(const dpp::snowflake guild_id, const std::string& input);
        void add_results_by_username(const dpp::snowflake guild_id, const std::string& input);
    };
}
